...
def upload_ultrasound_images(image_paths):
    images = []
...
def analyze_ultrasound_images(images, boxes, mm_per_pixel=0.1):
    ...
    # Debugging: Print lengths of inputs
    print(f"Number of images: {len(images)}")
    print(f"Number of bounding box sets: {len(boxes)}")
    for idx, box_set in enumerate(boxes):
        print(f"Image {idx + 1} has {len(box_set)} bounding boxes.")

    # Validate input lengths
    if len(images) != len(boxes):
        raise ValueError(
            f"Mismatch between the number of images ({len(images)}) and bounding box sets ({len(boxes)})."
        )

    # Process each image and its corresponding bounding boxes
    for idx, image in enumerate(images):
        if image is None:
            raise ValueError(f"Image at index {idx} is invalid. Check the image data or path.")

        tumor_details_per_image = []
        if idx >= len(boxes) or not boxes[idx]:  # Handle missing or empty bounding boxes
            findings += f"The ultrasound scan {idx + 1} reveals no suspicious regions.\n"
            tumor_details.append([])
            continue

        for box in boxes[idx]:  # Iterate through bounding boxes
            if len(box) != 4:
                raise ValueError(
                    f"Bounding box {box} at index {idx} is invalid. It should have 4 coordinates."
                )
        ...
        for i, detail in enumerate(tumor_details_per_image, start=1):
            findings += (f"- Region {i}: Size ({detail['width_mm']:.2f} mm x {detail['height_mm']:.2f} mm)\n")

        tumor_details.append(tumor_details_per_image)

    # Recommendations and Correlation
    if any(tumor_details):  # Add recommendations only if tumors are detected
        recommendations = (
            "* Recommended Tests:\n"
            ...
            "- Sonohysterography\n"
            "- Hysteroscopy\n\n"
            "Correlation: Since the scan is an Ultrasound, it is recommended to follow up with a Pelvic MRI or "
            "Sonohysterography for further evaluation of the detected regions."
        )
    else:
...
def generate_findings1(image_path, boxes):
